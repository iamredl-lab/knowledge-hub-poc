...
2. `pnpm install && pnpm fmt:check && pnpm lint && pnpm -r typecheck && pnpm -r test && pnpm -r build`
   must pass locally before a PR — this is exactly what CI checks. A pre-commit hook
   (husky + lint-staged) already runs `oxfmt`/`oxlint --fix` on staged files, so most
   formatting/lint issues are caught before they even reach a commit.
